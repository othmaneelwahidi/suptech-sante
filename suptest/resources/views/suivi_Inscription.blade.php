    ...
    <section id="contact" class="contact">
      <div class="container" data-aos="fade-up">
        ...
        @if(session('status'))
          ...
          <form id="regForm" action="{{ route('CheckUserInscrit' , ['slug' => App::getLocale()] ) }}" method="post" >
              @csrf
                <h1 id="register">{{__('messages.suiviInscr')}}  </h1>
                ...
                <div style="overflow:auto;" id="nextprevious">
                      ...
                      <button type="submit" style="float:right">{{__('messages.SuivantInsc')}}</button>
                </div>
            </form>
          </div>
      </div>
...
<script>
        $(".changeLang").change(function () {
            if ($(this).val() == 'fr') {
                window.location.href = "/fr/SuiviInscription";
            } else if ($(this).val() == 'ar') {
                window.location.href = "/ar/SuiviInscription";
            }
        });
</script>
